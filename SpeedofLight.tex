...
\section{Results}
	...
	The speed of light $c$ for each set was calculated using the mean deflections $s'_{cw}-s'_{ccw}$, the rotation speed difference $f_{cw} + f_{ccw}$, the distances $A$, $B$, and $D$, and Equation (\ref{eq:SpeedofLightCalc1}) derived in Section \ref{sec:Methods} and restated below as Equation (\ref{eq:speedoflight_calcs}).
